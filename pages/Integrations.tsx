...
const IntegrationCard: React.FC<IntegrationCardProps> = ({ name, logo, description, isConnected }) => (
    <div className="bg-[#111] p-6 rounded-lg border border-gray-800/50 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1 hover:bg-[#181818]">
        <img src={logo} alt={`${name} logo`} className="h-16 w-16 mb-4 object-contain" />
        <h3 className="text-xl font-bold">{name}</h3>
        <p className="text-sm text-gray-400 mt-2 mb-4 flex-grow">{description}</p>
        {isConnected ? (
...
const Integrations = () => {
    return (
        <div>
            ...
            <p className="text-gray-400 mb-6">Conecte suas ferramentas para centralizar e potencializar suas análises.</p>
            ...
            <div className="mt-8 bg-[#111] p-4 rounded-lg border border-dashed border-gray-700 flex items-center gap-4">
                ...
                <p className="text-gray-300">Não encontrou a integração que precisa? <a href="#" className="text-[#C5FF00] font-semibold hover:underline">Solicite uma nova integração.</a></p>
            </div>
        </div>
    );
};
